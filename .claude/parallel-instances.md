...
- **Instance 7 → Instance 2:** seed 4 tincture recipes (echinacea, valerian, elderberry, milk-thistle) on sample plants in `SampleData.swift`; literals already hand-coded in `Features/Recipes/RecipePreviewFixtures.swift` — port them onto the matching plants. Once landed, `RecipePreviewFixtures.allRecipes` collapses to `SampleData.plants.flatMap(\.recipes)`.
- **Instance 7 → Instance 2:** add a `RecipesRepository` protocol + Supabase live + mock impls for reading a standalone `recipes` table. Replaces the current `dependencies.plants.featured().flatMap(\.recipes)` path in `RecipesHomeView.loadRecipes()` once a real recipe corpus is seeded.
- **Instance 7 → Instance 2:** add `supabase/migrations/*_recipes.sql` + seed migration once `scripts/ingest-recipes/` produces a JSON corpus (150+ rows across 40+ herbs from Grieve's *A Modern Herbal* + King's *Dispensatory* + NCCIH safety overlay). Schema should mirror the `Recipe` Codable shape and FK `plant_id` to `plants.id`.
